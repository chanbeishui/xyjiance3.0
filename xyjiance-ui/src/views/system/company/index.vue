      ...
      >
         ...
         <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
            <template #default="scope">
               ...
               <el-button link type="primary" icon="Plus" @click="handleAdd(scope.row)" v-hasPermi="['system:company:add']">新增</el-button>
               <el-button v-if="scope.row.parentId != 0" link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:company:remove']">删除</el-button>
            </template>
         </el-table-column>
      </el-table>
...
import { listCompany, getCompany, delCompany, addCompany, updateCompany, listCompanyExcludeChild } from "@/api/system/company"
...
function getList() {
  ...
  listCompany(queryParams.value).then(response => {
    companyList.value = proxy.handleTree(response.data, "companyId")
    loading.value = false
  })
}
...
function handleAdd(row) {
  ...
  listCompany().then(response => {
    companyOptions.value = proxy.handleTree(response.data, "companyId")
  })
  if (row != undefined) {
...
function handleUpdate(row) {
  reset()
  listCompanyExcludeChild(row.companyId).then(response => {
    companyOptions.value = proxy.handleTree(response.data, "companyId")
  })
...
function handleDelete(row) {
  proxy.$modal.confirm('是否确认删除名称为"' + row.companyName + '"的数据项?').then(function() {
